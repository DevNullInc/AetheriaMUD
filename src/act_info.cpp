...
char *format_obj_to_char(OBJ_DATA *obj, CHAR_DATA *ch, bool fShort)
{
   ...
   if (IS_OBJ_STAT(obj, ITEM_GLOW))
      mudstrlcat(buf, "&G&W(&OG&Yl&wo&Ww&wi&Yn&Og&G&W) ", MAX_STRING_LENGTH);
   if (IS_OBJ_STAT(obj, ITEM_HUM))
      mudstrlcat(buf, "&G&W(&gH&Gu&wm&Wm&wi&Gn&gg&G&W) ", MAX_STRING_LENGTH);
...
void show_char_to_char_0(CHAR_DATA *victim, CHAR_DATA *ch)
{
   ...
   if (!IS_NPC(victim) && IS_SET(victim->pcdata->act2, ACT_GAGGED))
      mudstrlcat(buf, "&w&W(&rG&Ra&rg&Rg&re&Rd&w&W)&w ", MAX_STRING_LENGTH);
   if (!IS_NPC(victim) && IS_SET(victim->pcdata->act2, ACT_BOUND))
      mudstrlcat(buf, "&G&W(&gB&Go&wu&Gn&gd&G&W)&w ", MAX_STRING_LENGTH);
   ...
   if (victim && victim->position == victim->defposition && victim->long_descr[0] != '\0')
   {
      ...
      { // \033[1;31m
         ...
         mudstrlcpy(buf2, "\033[1;31m", MAX_STRING_LENGTH);
         mudstrlcat(buf, victim->long_descr, MAX_STRING_LENGTH);
         mudstrlcat(buf2, buf, MAX_STRING_LENGTH);
         send_to_char_noand(buf2, ch);
         show_visible_affects_to_char(victim, ch);
      }
      else
      {
         send_to_pager(victim->long_descr, ch);
         show_visible_affects_to_char(victim, ch);
      }
      return;
   }
...
void do_look(CHAR_DATA *ch, const char *argument)
{
   ...
   if (!IS_NPC(ch) && !IS_SET(ch->act, PLR_HOLYLIGHT) && !IS_AFFECTED(ch, AFF_TRUESIGHT) && !IS_AFFECTED(ch, AFF_INFRARED) && room_is_dark(ch->in_room))
   {
...
void do_who(CHAR_DATA *ch, const char *argument)
{
   ...
   for (d = last_descriptor; d; d = d->prev)
   {
      ...
      if ((d->connected != CON_PLAYING && d->connected != CON_EDITING) || (!can_see(ch, d->character) && IS_IMMORTAL(d->character)) || d->original)
         continue;
      wch = d->original ? d->original : d->character;
      if (wch->top_level < iLevelLower || wch->top_level > iLevelUpper || (fImmortalOnly && wch->top_level < LEVEL_IMMORTAL) || (fRaceRestrict && !rgfRace[wch->race]))
         continue;
      ...
      if (ch && ch->pcdata && ch->pcdata->clan && wch->pcdata->clan && (ch->pcdata->clan == wch->pcdata->clan || ch->top_level >= LEVEL_IMMORTAL))
      {
   ...
   if (first_newbie)
   {
      if (!ch)
         fprintf(whoout, "%s",
                 htmlcolor("&G===&B[ &PFotE &B]&G===&B[ &WMortals &B]&G=====================================================&B[]&W\n\n"));
      else
   ...
   if (first_mortal)
   {
      if (!ch)
         fprintf(whoout, "%s",
                 htmlcolor("&z(  &rPlayers&z  )&R-_-&r^^-_-^^-&R_-&r^^-_-^^-_-^^-&R_-^^-&r_-^^-_-&R^^&z(=====+ &rFall of the Empire &z+====&z=)&w\n\n"));
      else
   ...
   if (first_imm)
   {
      if (!ch)
         fprintf(whoout, "%s",
                 htmlcolor("&z( &rImmortals&z )&R-_&r-^^-_-&R^&r-_-^^-_-^^&R-_-^^&r-_-^^-_-^^-&R_-^^&z(=====+ &rFall of the Empire &z+====&z=)&w\n\n"));
      else
   ...
   {
      fprintf(whoout,
              htmlcolor("&z( &R%d &rvisible &zplayer%s. &R%d &rtotal&z player%s. )&R-_&r-^^-_-&R^^&r-_-^^-_-^^&R-_-^^&r-_-^^-_-^^-&R^^-\n"),
...
void do_setrank(CHAR_DATA *ch, const char *argument)
{
   ...
   argument = smash_tilde_copy(argument);
   snprintf(buf, MAX_INPUT_LENGTH, "%s", argument);
...
void do_where(CHAR_DATA *ch, const char *argument)
{
   ...
   if (arg[0] == '\0')
   {
      ...
      for (d = first_descriptor; d; d = d->next)
         if ((d->connected == CON_PLAYING || d->connected == CON_EDITING) && (victim = d->character) != NULL && !IS_NPC(victim) && victim->in_room && (victim->in_room->area == ch->in_room->area || get_trust(ch) >= LEVEL_IMMORTAL) && can_see(ch, victim))
         {
            found = TRUE;
   ...
   if (argument[0] == '\0')
   {
      ...
      for (sn = 0; sn < top_sn; sn++)
      {
         ...
         pager_printf(ch, "&B[&W%3d%%&B] %s%-18.18s ",
                      ...
                      skill_table[sn]->alignment == 0 ? "&G" : skill_table[sn]->alignment == -1001 ? "&R"
                                                                                                   : "&B",
                      skill_table[sn]->name);
         if (++col % 3 == 0)
            send_to_pager("\r\n", ch);
...
void do_teach(CHAR_DATA *ch, const char *argument)
{
   ...
   else
   {
      ...
      else
      {
         ...
         snprintf(buf, MAX_STRING_LENGTH, "You teach %s $T.", PERS(victim, ch));
         act(AT_ACTION, buf, ch, NULL, skill_table[sn]->name, TO_CHAR);
         snprintf(buf, MAX_STRING_LENGTH, "%s teaches you $T.", PERS(ch, victim));
...
void do_commands(CHAR_DATA *ch, const char *argument)
{
   ...
   if (argument[0] == '\0')
   {
      for (hash = 0; hash < 126; hash++)
         for (command = command_hash[hash]; command; command = command->next)
            if (command->level < LEVEL_HERO && command->level <= get_trust(ch) && !IS_CMD_MPROG(command))
            {
   ...
   else
   {
      ...
      for (hash = 0; hash < 126; hash++)
         for (command = command_hash[hash]; command; command = command->next)
            if (command->level < LEVEL_HERO && command->level <= get_trust(ch) && !str_prefix(argument, command->name) && !IS_CMD_MPROG(command))
            {
...
void do_channels(CHAR_DATA *ch, const char *argument)
{
   ...
   else
   {
      ...
      if (!str_cmp(arg + 1, "auction") && get_trust(ch) > 2 && !NOT_AUTHED(ch))
         ...
         bit = CHANNEL_OOC;
      else if (!str_cmp(arg + 1, "clan") && !IS_NPC(ch) && ch->pcdata->clan && ch->pcdata->clan->clan_type != CLAN_GUILD)
         bit = CHANNEL_CLAN;
         ...
         for (sn = 0; sn < top_sn; sn++)
         {
            ...
            if (i == skill_table[sn]->min_level)
            {
               pager_printf(ch, "&B(&W%2d&B)%s %-18.18s&W  ",
                            i, skill_table[sn]->alignment >= 1001 ? "&B" : skill_table[sn]->alignment <= -1001 ? "&R"
                                                                                                               : "&W",
                            skill_table[sn]->name);
...
void do_whois(CHAR_DATA *ch, const char *argument)
{
   ...
   mudstrlcpy(buf, "0.", MAX_STRING_LENGTH - 30);
...
void do_introduce(CHAR_DATA *ch, const char *argument)
{
   ...
   if (!str_cmp(arg1, "all"))
   {
      ...
      for (rch = ch->in_room->first_person; rch; rch = rch->next_in_room)
      {
         ...
         if (can_see(ch, rch))
         {
            snprintf(buf, MAX_STRING_LENGTH, "__%s ", rch->name);
