...
void do_induct(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("induct", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_outcast(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("outcast", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_showclan(CHAR_DATA *ch, const char *argument)
{
   ...
   ch_printf(ch, "%s      : %s\r\nFilename: %s\r\n",
             clan->clan_type == CLAN_CRIME ? "Crime Family " : clan->clan_type == CLAN_GUILD ? "Guild "
                                                                                             : "Organization ",
...
void do_gatherclans(CHAR_DATA *ch, const char *argument)
{
   ...
   for (clan = first_clan; clan; clan = clan->next)
   {
      if (clan->clan_type == CLAN_CRIME || clan->clan_type == CLAN_GUILD || clan->clan_type == CLAN_SUBCLAN || clan->clan_type == CLAN_CORPORATION || clan->clan_type == CLAN_PLAIN)
         continue;
   ...
   for (clan = first_clan; clan; clan = clan->next)
   {
      if (clan->clan_type != CLAN_CRIME && clan->clan_type != CLAN_GUILD && clan->clan_type != CLAN_CORPORATION && clan->clan_type != CLAN_PLAIN)
         continue;
      if (!str_cmp(clan->name, "Neutral"))
         continue;
...
void do_drag(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((pexit = get_exit(ch->in_room, exit_dir)) == NULL)
      nogo = TRUE;
   else if (IS_SET(pexit->exit_info, EX_CLOSED) && (!(IS_AFFECTED(ch, AFF_PASS_DOOR) && IS_AFFECTED(victim, AFF_PASS_DOOR)) || IS_SET(pexit->exit_info, EX_NOPASSDOOR)))
      nogo = TRUE;
   if (nogo)
   {
...
void do_clan_withdraw(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("withdraw", ch->pcdata->bestowments)) || !str_cmp(ch->name, ch->pcdata->clan->leader))
      ;
   else
   {
...
void do_clanbuytroops(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("clanbuyship", ch->pcdata->bestowments)) || !str_cmp(ch->name, ch->pcdata->clan->leader))
      ;
   else
   {
...
void do_empower(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("withdraw", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->leader))
      ;
   else
   {
...
void do_outlaw(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("outlaw", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_unoutlaw(CHAR_DATA *ch, const char *argument)
{
   ...
   if ((ch->pcdata && ch->pcdata->bestowments && is_name("unoutlaw", ch->pcdata->bestowments)) || !str_cmp(ch->name, clan->leader) || !str_cmp(ch->name, clan->number1) || !str_cmp(ch->name, clan->number2))
      ;
   else
   {
      send_to_char("Huh?\r\n", ch);
...
void do_clans(CHAR_DATA *ch, const char *argument)
{
   ...
   for (clan = first_clan; clan; clan = clan->next)
   {
      ...
      ch_printf(ch, "&z| &WOrganization: &w%-25.25s &WType: &w%-15.15s &z|\r\n",
                clan->name, clan->clan_type == 0 ? "Government" : clan->clan_type == 1 ? "Crime Syndicate"
   ...
   for (clan = first_clan; clan; clan = clan->next)
   {
      ...
      if (clan->clan_type != 1)
         ch_printf(ch, "&z|       &W%6.6s: &w%-15.15s                                 &z|\r\n",
                   clan->clan_type == 1 ? "Leader" : clan->clan_type == 2 ? "Leader"
...
void do_members(CHAR_DATA *ch, const char *argument)
{
   ...
   snprintf(color, MAX_INPUT_LENGTH, "%s", !str_cmp(clan->name, "The Empire") ? "&R" : !str_cmp(clan->name, "The New Republic") ? "&B"
                                                                                                                                : "&G");
