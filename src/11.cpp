...
const char *primary_beam_name(SHIP_DATA *ship)
{
   if (ship->primaryCount != 0)
   {
      snprintf(bname, MAX_STRING_LENGTH, "%d %s", ship->primaryCount,
               (ship->primaryType == SINGLE_LASER && ship->primaryCount == 1) ? "Single-laser cannon" : (ship->primaryType == SINGLE_LASER && ship->primaryCount != 1) ? "Single-laser cannons"
...
const char *secondary_beam_name(SHIP_DATA *ship)
{
   ...
   if (ship->secondaryCount != 0)
   {
      snprintf(bname, MAX_STRING_LENGTH, "%d %s", ship->secondaryCount,
               (ship->secondaryType == SINGLE_LASER && ship->secondaryCount == 1) ? "Single-laser cannon" : (ship->secondaryType == SINGLE_LASER && ship->secondaryCount != 1) ? "Single-laser cannons"
...
void explode_emissile( CHAR_DATA * ch, ROOM_INDEX_DATA * proom, int mindam, int maxdam, bool incendiary )
{
   ...
   void do_unlink(CHAR_DATA * ch, const char *argument)
   {
      ...
      if (str_cmp(argument, "primary") && str_cmp(argument, "secondary") && str_cmp(argument, "launchers") && str_cmp(argument, "all"))
      {
