...
void do_recho(CHAR_DATA *ch, const char *argument)
{
   ...
   one_argument(argument, arg);
   if (!str_cmp(arg, "Thoric") || !str_cmp(arg, "Dominus") || !str_cmp(arg, "Circe") || !str_cmp(arg, "Haus") || !str_cmp(arg, "Narn") || !str_cmp(arg, "Scryn") || !str_cmp(arg, "Blodkai") || !str_cmp(arg, "Damian"))
   {
      ch_printf(ch, "I don't think %s would like that!\r\n", arg);
      return;
   }
...
void do_mstat(CHAR_DATA *ch, const char *argument)
{
   ...
   ch_printf(ch, "&W&z| &GName&W: %-12.12s     &GLastname&W: %-12s      &GClan&W: %-19s &z|\r\n", victim->name,
             (IS_NPC(victim) || !victim->pcdata->last_name) ? "(none)" : victim->pcdata->last_name, (IS_NPC(victim) || !victim->pcdata->clan) ? "(none)" : victim->pcdata->clan->name);
   ch_printf(ch,
   ...
   ch_printf(ch, "&W&z|       &GBank&W: %-10ld            &GAC&W: %-5d             &GMana&W: %5d/%-5d    &z|\r\n",
             (IS_NPC(victim) || !victim->pcdata->bank) ? 0 : victim->pcdata->bank, GET_AC(victim), victim->mana, victim->max_mana);
...
void do_users(CHAR_DATA *ch, const char *argument)
{
   ...
   for (d = first_descriptor; d; d = d->next)
   {
      ...
      {
         if ((get_trust(ch) >= LEVEL_SUPREME || (d->character && can_see(ch, d->character))) && (!str_prefix(argument, d->host) || (d->character && !str_prefix(argument, d->character->name))))
         {
            count++;
...
void do_fslay(CHAR_DATA *ch, const char *argument)
{
   ...
   if (get_trust(victim) >= get_trust(ch))
   {
      send_to_char("If they have any sort of mental capacity, they would realize you would be unable to slay them anyways. So why don't you just stop what you're doing and go play with some Duplo blocks, before we all succumb to complete and utter mindlessness from your total lack of intelligence.\r\n",
                   ch);
      return;
   }
...
void ostat_plus(CHAR_DATA *ch, OBJ_DATA *obj)
{
   ...
   case ITEM_CONTAINER:
      ...
      ch_printf(ch, "%s\r\n",
                obj->value[0] < 76 ? "Small capacity" : obj->value[0] < 150 ? "Small to medium capacity"
